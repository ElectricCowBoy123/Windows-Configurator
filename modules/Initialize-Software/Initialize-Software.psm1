...
function Install-ScheduledTasks() {
    ...
    foreach ($file in $ahkFiles) {
        ...
        $taskExists = -not (& "$env:SystemRoot\System32\schtasks.exe" /Query /TN $taskPath 2>&1 | Select-String "ERROR:")
        ...
        if (-not $taskExists) {
            ...
            & "$env:SystemRoot\System32\schtasks.exe" /Create /TN $taskPath /XML $tempXmlPath /F | Out-Null
